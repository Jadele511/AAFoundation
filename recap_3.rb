...
def bi_prime?(n)
    (2...n).each do |fact|
        otherFact = n / fact
        return true if prime?(fact) && prime?(otherFact) && fact * otherFact == n   
    end
    false
...
class String
    def select(&prc)
        return '' if prc.nil?
        res = ''
        self.each_char {|ch| res += ch if prc.call(ch)}
        res 
    end

    def map!(&prc)
        self.each_char.with_index {|ch, i| self[i] = prc.call(ch, i) if prc.call(ch, i)}
    end
end
...
def prime_factorization(n)
    ...
    res = []
    (2...n/2).each do |fact|
        if n % fact == 0 && prime?(fact)
            res << fact 
            otherFact = n / fact
            res += prime_factorization(otherFact)
            return res 
        end
    end
end
