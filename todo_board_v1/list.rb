...
class List
    ...
    def print
        ...
        p ' ' * 20 + @label + '' * 20
        p '-' * 50
        p "#{'Index'.ljust(7)}|  #{'Item'.ljust(20)}|  #{'Deadline'.ljust(13)}" 
        @items.each_with_index do |item, i|
            p "#{i.to_s.ljust(7)}|  #{item.title.ljust(20)}|  #{item.deadline.ljust(13)}"
        end
    ...
    def print_full_item(idx)
        p '-' * 50
        if valid_index?(idx)
            p @items[idx].title.ljust(35) + @items[idx].deadline
