...
def first_index(arr, &prc)
    arr.each_index {|i| return i if prc.call(arr[i])}
    nil
end
# Examples
...
def proctition(arr, &prc)
    ...
    right = []
    arr.each do |ele|
        left << ele if prc.call(ele)
        right << ele if !prc.call(ele)
    end
    left + right
end
# Examples
...
def proctition_platinum(arr, *prcs)
    ...
    (0...prcs.length).each do |i|
        arr.each do |ele|
            hash[i+1] << ele if prcs[i].call(ele) && !hash.values.flatten.include?(ele)
        end
    end
    hash
end
